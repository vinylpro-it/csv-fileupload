...
class SCREENCUTTINGProcessor(BaseProcessor):
    ...
    def get_table_name(self):
        return "screencutting" #SCREENCUTTING
    ...
    def process(self, file_path: Path, move_dir: Path) -> bool:
        try:
            self.logger.info(f"Processing SCREENCUTTING file: {file_path}")
            ...
            self.logger.error(f"Error processing SCREENCUTTING file {file_path}: {str(e)}")
            return False
        finally:
            self.disconnect()
    ...
    def upload_csv_data(self, table_name, csv_file_path):
        ...
        try:
            ...
            csv_headers = [
                ...
                'WINDOWS_SIZE', 'ITEM_NAME', 'STOP LINE', 'COMPANY', 'PO', 
                'DATE', 'TIME', 'USER', 'ID'
            ]
