...
class STOPCUTTINGProcessor(BaseProcessor):
    ...
    def get_table_name(self):
        return "stopcutting" #STOPCUTTING
    ...
    def process(self, file_path: Path, move_dir: Path) -> bool:
        try:
            self.logger.info(f"Processing STOPCUTTING file: {file_path}")
            ...
            self.logger.error(f"Error processing STOPCUTTING file {file_path}: {str(e)}")
            return False
        finally:
            self.disconnect()
